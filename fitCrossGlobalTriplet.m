function globalFit = fitCrossGlobalTriplet(tblCell, linkParams, initialGuessCell)

% tblCell: cell array of tables

% linkParams: logical [1x5] → which parameters are global
% like this: linkParams = [true false true false false false];
% order of params: [N0 tauD0 k0 trpFrac trpTau offset]

%sample usage: Ffits = fitCrossGlobalTriplet(FData, [false true false true true])

% initialGuessCell: cell array of initial guesses per dataset (optional)
%Like this:
%initialGuessCell = {
%    [1.1  0.01  5.0  0.1  1e-6];   % <-- global guesses taken from here
%    [999  0.02  999  0.05 2e-6];   % global entries ignored
%    [999  0.03  999  0.02 3e-6]
%};
...
for s = 1:nSets
    ...
    if isempty(initialGuessCell)
        ...
        trpTau = 1e-6;
        offset=0.1;
        guess = [N0 tauD0 k0 trpFrac trpTau offset];
    else
...
lbSingle = [1e-5 1e-12 1 0 0 0];
ubSingle = [1e4 100 50 0.01 1e-02 1e-02];
...
for s = 1:nSets
    ...
    globalFit.dataset(s).trpFrac     = ps(4);
    globalFit.dataset(s).trpFrac_err = errs(4);
    
    globalFit.dataset(s).trpTau     = ps(5);
    globalFit.dataset(s).trpTau_err = errs(5);

    globalFit.dataset(s).offset     = ps(6);
    globalFit.dataset(s).offset_err = errs(6);
end
